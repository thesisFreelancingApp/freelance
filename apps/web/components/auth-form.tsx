...
export default function AuthForm() {
  ...
  const handleAuth = async (authType: "google" | "email") => {
    ...
    try {
      const { data } = await axiosInstance.post(`${domaine}/auth/${authType}`, {
        email,
      ...
      <Card className="w-full max-w-sm">
        ...
        <CardContent>
          <CardDescription className="text-center">
            Vérifiez votre email pour confirmer votre inscription et vous
            connecter.
          </CardDescription>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="w-full max-w-sm">
      <CardHeader>
        <CardTitle>{isLoginView ? "Connexion" : "Inscription"}</CardTitle>
        <CardDescription>
          {isLoginView
